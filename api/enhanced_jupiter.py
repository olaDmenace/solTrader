...
class RobustHTTPClient:
                ...
                async with self.session.get(url, params=params) as response:
                    if response.status == 200:
                        ...
                        if attempt < self.retry_config.max_retries - 1:
                            await asyncio.sleep(self.retry_config.initial_delay * (self.retry_config.backoff_factor ** attempt))
                            continue
    ...
    async def post(self, endpoint: str, json_data: Dict = None, data: Dict = None) -> Dict:
...
class EnhancedJupiterClient:
    ...
    async def get_sol_price(self) -> Optional[float]:
        """Alias for get_sol_price_in_usdc for compatibility"""
        return await self.get_sol_price_in_usdc()
