"""
Position Models - MIGRATED from src/trading/position.py

MIGRATION NOTE: Extracted from src/trading/position.py for Day 3 data model consolidation
CRITICAL: All position management logic PRESERVED 100% - no algorithm modifications
...
# Numpy for calculations (with fallback)
try:
    import numpy as np
except ImportError:
...
class Position:
    ...
    def update_price(self, new_price: float, volume: Optional[float] = None) -> None:
        ...
        try:
            ...
            # Record position update metrics with Prometheus
            try:
                metrics = get_metrics()
                if metrics:
                    metrics.record_position_update(self.token_address, new_price, self.unrealized_pnl)
            except Exception as e:
                logger.debug(f"[METRICS] Position update recording failed (non-critical): {e}")

        except Exception as e:
            # Capture error with Sentry for professional error tracking
            capture_api_error(
                error=e,
                api_name="position_manager",
                endpoint="update_price",
                context={"token_address": self.token_address, "new_price": new_price}
            )
            logger.error(f"[POSITION] Error updating price for {self.token_address}: {e}")
    ...
    def should_close(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            capture_api_error(
                error=e,
                api_name="position_manager",
                endpoint="should_close",
                context={"token_address": self.token_address}
            )
            logger.error(f"[POSITION] Error checking close conditions for {self.token_address}: {e}")
            return False, ""
...
class PositionManager:
    ...
    async def open_position(
        ...
        try:
            ...
            # Record position open metrics with Prometheus
            try:
                metrics = get_metrics()
                if metrics:
                    metrics.record_position_open(token_address, entry_price, size)
            except Exception as e:
                logger.debug(f"[METRICS] Position open recording failed (non-critical): {e}")
            ...
            capture_api_error(
                ...
                context={"token_address": token_address, "size": size, "entry_price": entry_price}
            )
    ...
    async def close_position(self, token_address: str, reason: str = "manual") -> bool:
        ...
        try:
            ...
            if success:
                ...
                # Record position close metrics with Prometheus
                try:
                    metrics = get_metrics()
                    if metrics:
                        metrics.record_position_close(token_address, position.current_price, position.unrealized_pnl)
                except Exception as e:
                    logger.debug(f"[METRICS] Position close recording failed (non-critical): {e}")
            ...
            capture_api_error(
                ...
                context={"token_address": token_address, "reason": reason}
            )
    ...
    def update_token_balance(self, token_address: str, token_balance: float) -> bool:
        ...
        try:
            ...
            capture_api_error(
                error=e,
                api_name="position_manager",
                endpoint="update_token_balance",
                context={"token_address": token_address, "token_balance": token_balance}
            )
