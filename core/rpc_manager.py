...
MIGRATION NOTE: Moved from src/api/multi_rpc_manager.py
...
class MultiRPCManager:
    ...
    def __init__(self):
        ...
        # Initialize Jito as premium MEV-protected provider
        self._setup_jito_provider()
    ...
    def _setup_jito_provider(self):
        """Setup Jito MEV-protected RPC provider"""
        try:
            logger.info("[RPC] Initializing Jito MEV protection provider...")
            self.jito_provider = JitoRPCProvider()
            
            # Get Jito provider configuration
            import asyncio
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            try:
                jito_config = loop.run_until_complete(self.jito_provider.get_jito_rpc_config())
            finally:
                loop.close()
            
            # Add Jito as a premium provider
            jito_rpc_provider = RPCProvider(
                name=jito_config["name"],
                url=jito_config["url"],
                max_requests_per_second=jito_config["max_requests_per_second"],
                max_requests_per_day=jito_config.get("max_requests_per_day"),
                priority=0  # Highest priority for MEV protection
            )
            
            self.providers[jito_config["name"]] = jito_rpc_provider
            logger.info(f"[RPC] Jito MEV provider registered: {jito_config['name']}")
            logger.info(f"[RPC] Jito features: {jito_config.get('features', [])}")
            
        except Exception as e:
            logger.warning(f"[RPC] Jito MEV provider initialization failed: {e}")
            logger.info(f"[RPC] Continuing with standard RPC providers")
            self.jito_provider = None
        ...
        for attempt in range(max_retries):
            try:
                ...
                if self.current_provider:
                    ...
                    capture_api_error(
                        error=e,
                        api_name=self.current_provider,
                        endpoint="rpc_operation",
                        context={
                            "attempt": attempt + 1,
                            "max_retries": max_retries,
                            "execution_time": execution_time,
                            "provider_stats": self.providers[self.current_provider].stats.__dict__
                        }
                    )
    ...
    async def health_check_all_providers(self):
        ...
        async def check_provider(name: str, provider: RPCProvider):
            try:
                ...
                # Capture health check failures for monitoring
                capture_api_error(
                    error=e,
                    api_name=name,
                    endpoint="health_check",
                    context={"provider_url": provider.url[:50] + "..."}
                )
                return False
            finally:
                if 'client' in locals():
    ...
    async def close_all_clients(self):
        ...
        # Close Jito provider separately
        if self.jito_provider:
            await self.jito_provider.close()
            logger.info("[RPC] Jito MEV provider closed")
    ...
    def get_stats(self) -> Dict[str, Dict[str, Any]]:
        ...
        for name, provider in self.providers.items():
            ...
            # Add Jito-specific features for MEV protection provider
            if name.startswith('jito_') and self.jito_provider:
                provider_stats.update({
                    'mev_protection': True,
                    'priority_ranking': provider.priority,
