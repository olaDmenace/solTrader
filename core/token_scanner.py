"""
Enhanced Token Scanner - CRITICAL ALPHA COMPONENT

MIGRATION NOTE: Moved from src/enhanced_token_scanner.py
CRITICAL: 40-60% approval rate MUST BE PRESERVED - no algorithm modifications
Core functionality preserved 100% - enhanced with Sentry error tracking and Prometheus metrics
"""
...
class EnhancedTokenScanner:
    def __init__(self, settings: Settings, analytics=None):
        ...
        # PHASE 2 DAY 2: Smart Age Extension for high-momentum tokens
        self.max_token_age_hours = 168  # EXTENDED to 7 days (168 hours) to capture high-momentum older tokens
        self.extended_age_hours = 720   # 30 days for exceptional momentum (25%+ tokens)
        ...
        logger.info(f"Max age: {self.max_token_age_hours} hours (extended to {self.extended_age_hours}h for 25%+ momentum)")
    ...
    async def _scan_loop(self):
        ...
        while self.is_running:
            try:
                ...
                # Capture critical scanner errors with Sentry
                capture_api_error(
                    error=e,
                    api_name="EnhancedTokenScanner",
                    endpoint="scan_loop",
                    context={
                        "daily_stats": self.daily_stats,
                        "approval_rate": self.daily_stats.get('approval_rate', 0)
                    }
                )
    ...
    async def _perform_full_scan(self):
        ...
        try:
            ...
            # Record critical approval rate metrics with Prometheus
            try:
                metrics = get_metrics()
                metrics.update_token_approval_rate("enhanced_scanner", self.daily_stats.get('approval_rate', 0))
                metrics.record_momentum_detection("scan_complete")
                logger.debug(f"[METRICS] Approval rate recorded: {self.daily_stats.get('approval_rate', 0):.1f}%")
            except Exception as e:
    ...
    async def _evaluate_token(self, token: TokenData) -> Optional[ScanResult]:
        ...
        if not high_momentum:
            ...
            if token.liquidity < liquidity_threshold:
                return None
            reasons.append(f"Liquidity: ${token.liquidity:,.0f} USD")
            ...
            # PHASE 2 DAY 2: Smart Age Extension - High momentum tokens get extended age allowance
            exceptional_momentum = token.price_change_24h >= 25.0  # 25%+ momentum tokens
            if exceptional_momentum:
                age_limit_hours = self.extended_age_hours  # 30 days for exceptional momentum (25%+)
                reasons.append("Exceptional momentum - extended age allowance")
            elif medium_momentum:
                age_limit_hours = self.max_token_age_hours * 1.5  # Medium momentum gets 1.5x extension
            else:
                age_limit_hours = self.max_token_age_hours  # Standard 7-day limit
