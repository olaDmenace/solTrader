...
MIGRATION NOTE: Moved from src/arbitrage/real_dex_connector.py
...
class RealDEXConnector:
    ...
    async def start(self):
        ...
        try:
            ...
            # Capture initialization errors with Sentry
            capture_api_error(
                error=e,
                api_name="DEXConnector",
                endpoint="initialize",
                context={"dex_endpoints": list(self.dex_endpoints.keys())}
            )
            return False
    ...
    async def get_quote(
        ...
        try:
            ...
            # Capture DEX API errors with Sentry
            capture_api_error(
                error=e,
                api_name=dex.value,
                endpoint="get_quote",
